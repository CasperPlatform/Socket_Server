...
class moveFlag(Enum):
    Camera = 'C'
class cameraFlag(Enum):
    Up = 'U'
    Down = 'D'
    Idle = 'I'
class cameraAngleFlag(Enum):
    Right = 'R'
    Left  = 'L'
    Idle  = 'I'
...
class CasperProtocol(Protocol):
    ...
    def dataReceived(self, data):
        ...
        if datarec[0] != ord(cameraFlag.Camera):
             print repr(datarec[0])
             print 'unknown cameraFlag...aborting'
             return
        else:
            tf = cameraFlag.Camera
            print 'this is a moveMsg'
        ...
        if datarec[17] == ord(cameraFlag.Up) or datarec[17] == ord(cameraFlag.Down) or datarec[17] == ord(cameraFlag.Idle):
            if datarec[17] == ord(cameraFlag.Up):
                print 'direction: Up'
                cfu = cameraFlag.Up
            if datarec[17] == ord(cameraFlag.Down):
                print 'direction: Back'
                cfd = cameraFlag.Down
            if datarec[17] == ord(cameraFlag.Idle):
                print 'direction: Idle'
                cfi = cameraFlag.Idle
        else:
            print 'unknown cameraFlag..aborting'
            return
        if datarec[18] == ord(cameraAngleFlag.Right) or datarec[18] == ord(cameraAngleFlag.Left) or datarec[18] == ord(cameraAngleFlag.Idle):
            if datarec[18] == ord(cameraAngleFlag.Right):
                print 'cameraAngle: Right'
                cafr = cameraAngleFlag.Right
            if datarec[18] == ord(cameraAngleFlag.Left):
                print 'cameraAngle: Left'
                cafl = cameraAngleFlag.Left
            if datarec[18] == ord(cameraAngleFlag.Idle):
                print 'cameraAngle: Idle'
                cafi = cameraAngleFlag.Idle
        else:
            print 'unknown cameraAngleFlag..aborting'
...
SerialPort(USBclient(), '/dev/cu.wchusbserial1410', reactor, baudrate='9600')
